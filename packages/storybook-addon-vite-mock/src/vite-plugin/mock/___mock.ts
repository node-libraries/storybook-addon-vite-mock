...
const createFunction = (key: string, original: AnyFunction): AnyFunction => {
  if (
...
const getSymbol = (func: AnyFunction): symbol | undefined => {
  return Object.getOwnPropertyDescriptor(func, '___symbol')?.value;
};

const funcMap: Record<symbol, { original: AnyFunction; custom: AnyFunction }> = {};
const ___setMock = <T extends AnyFunction>(func: T, custom: T): T => {
...
const ___getOriginal = <T extends AnyFunction>(func: T): T => {
  const key = getSymbol(func);
  if (!key) throw new Error(`Function is not a mock '${func.name}'`);
  return funcMap[key].original as T;
};
...
const ___createMock = (exp: Record<string, unknown>) => {
  const v = Object.entries(exp).map(([key, original]) => {
    if (typeof original === 'function' && !getSymbol(original as AnyFunction)) {
      const func = createFunction(key, original as AnyFunction);
      return [key, func];
    }
    return [key, original];
  });
...
const ___createCommonMock = (exp: NodeJS.Module['exports']) => {
  ...
  if (typeof exp === 'function') {
    const func = createFunction(exp.name, exp as AnyFunction);
